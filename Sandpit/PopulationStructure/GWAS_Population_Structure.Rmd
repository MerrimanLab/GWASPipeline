...
working_directory <- "/mnt/DataDrive/MedicagoFiles/Analyses/"
...
Review the cell below nd run (no changes necessary).  
...
We are not going to be quite as stringent as this. We will consider a sliding window of 3 kilobases, with a slide of 2 kilobases and R2 threshold of 0.5.  
...
```{r}
genotype_directory <- sprintf("%splink_genotype_files/", working_directory)
...
snp_map <- list()
for (idx in 1:length(genotype_files)) {
...
for (genotype in genotype_files) {
    ...
    ", plink_, 
       ...
       snp_map[[filename]],
       spritnf("%s%s_ld_filtered", working_directory, filename))
...
filtered_files <- list.files(working_directory, pattern = "ld_filtered", full.names = TRUE)
write(filtered_files[4:length(filtered_files)], merge_list)
...
", plink_,
   first_file,
   merge_list,
   sprintf("%smerged_genotypes", working_directory)
)
...
", plink_, 
   ...
   sprintf("%spopulation_structure", working_directory)
)
