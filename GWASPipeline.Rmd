...
for (trait_ in trait_files) {
    ...
    for (geno_ in genotype_files) {
        ...
        assoc_results <- sprintf("%s/ScratchSpace/%s_%s", home_, chromosome, lcl_trait)
        ...
        cmd <- sprintf("%s --bfile %s --pca 10 --out population_structure", plink_, "qc_genotypes")
        system(cmd)
    ...
    merged_results <- rbindlist(lapply(list.files(sprintf("%s/ScratchSpace", home_), pattern = ".assoc.linear$"),
                                       function (x) {
                                           tmp <- fread(x)
                                           return (tmp[TEST == "ADD"])
                                       }))
